...
def make_profile_text(df: pd.DataFrame) -> pd.Series:
    # Compose a short behavior/profile summary for BERT
    parts = []
...
def load_support_csv(optional_path="support_interactions.csv"):
    """
    Optional: a CSV with columns:
      customerID, timestamp, text
    We'll aggregate per customer and sentiment-score later.
    If file doesn't exist, return None.
    """
    try:
        sup = pd.read_csv(optional_path)
        needed = {'customerID','text'}
        if not needed.issubset(set(sup.columns)): return None
        return sup
    except Exception:
        return None
